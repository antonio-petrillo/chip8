...
load_rom :: proc(chip: ^Chip8, path: string) {
	...
	if os.close(fd) != os.ERROR_NONE {
		panic(
			"Can't close rom file, yeah I know a bit exagerate but please nun scassare le pallucce",
		)
	}
...
execute :: proc(chip: ^Chip8, instr: u16) {
	x := u8(instr & 0x0F00 >> 8)
	y := u8(instr & 0x00F0 >> 4)
	n := u8(instr & 0x000F)
	kk := u8(instr & 0x00FF)
	...
	case 0x8000:
		...
		case 0x5:
			log.infof(
				"Execute := SUB Vx, Vy [0x%4x], x := %1x, y := %1x, VF not borrow",
				instr,
				x,
				y,
			)
		...
		case 0x7:
			log.infof(
				"Execute := SUBN Vx, Vy [0x%4x], x := %1x, y := %1x, VF not borrow",
				instr,
				x,
				y,
			)
	...
	case 0xC000:
		...
		log.infof(
			"Execute := RND Vx, byte [0x%4x], V0 := %2x, byte := %2x, rng := %2x, res := %2x",
			instr,
			chip.V[0],
			kk,
			rng_byte,
...
update_keypad :: proc(chip: ^Chip8) {
    ...
    for key, idx in keys {
        if rl.IsKeyPressed(key) {
            chip.keypad[idx] = true
        }
        if rl.IsKeyReleased(key) {
            chip.keypad[idx] = false
        }
    }
}
